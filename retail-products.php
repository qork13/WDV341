    ...
    <section>
        ...
        <div class="productBlock">
            <div class="productImage">
                <image src="productImages/<?=$result['product_image']?>">
            </div>
            <p class="productName"><?=$result['product_name']?></p>
            <p class="productDesc"><?=$result['product_description']?></p>
            <p class="productPrice"><?=$result['product_price']?></p>
            <!-- The productStatus element should only be displayed if there is product_status data in the record -->
            <?php 
                if($result['product_status'] != '') { ?>
            <p class="productStatus"><?=$result['product_status']?>!</p> 
            <?php }  ?> 
            <?php
                if($result['product_inStock'] < 10) { ?>
            <p class="productLowInventory">Only <?=$result['product_inStock']?> left!</p>
            <?php } else { ?>        
            <p class="productInventory"><?=$result['product_inStock']?> in Stock</p>
            <?php } ?>
        </div>
        <?php } 
            $conn = null; ?>
